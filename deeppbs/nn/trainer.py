...
class Trainer(object):
    def __init__(self, model, nc, optimizer, criterion, 
            ...
            uq_local_loss_weight=0.0, uq_global_loss_weight=0.0,
            uq_global_target="mae_pcc"
            ):
        # parameters
        self.model = model
        self.nc = nc
        ...
        self.uq_local_loss_weight = uq_local_loss_weight
        self.uq_global_loss_weight = uq_global_loss_weight
        self.uq_global_target = uq_global_target
    ...
    def optimizer_step(self, batch_data, epoch, use_mask=True, use_weight=True, weight=None,
        ...
        if not valid:
            ...
            self.model.eval()
        model_out = self.model(batch_data['batch'])
        if isinstance(model_out, dict):
            output = model_out["logits"]
            local_log_var = model_out.get("local_log_var", None)
            local_uq_logits = model_out.get("local_uq_logits", None)
            global_quality = model_out.get("global_quality", None)
        else:
            output = model_out
            local_log_var = None
            local_uq_logits = None
            global_quality = None
        ...
        if ((local_log_var is not None) or (local_uq_logits is not None)) and (self.uq_local_loss_weight > 0):
            pred_prob = torch.softmax(output[out_mask], dim=1)
            true_prob = y[y_mask]
            pos_mae = torch.abs(pred_prob - true_prob).sum(dim=1).detach()
            if local_uq_logits is not None:
                bin_edges = torch.tensor(
                    [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0],
                    dtype=pos_mae.dtype,
                    device=pos_mae.device
                )
                bin_target = torch.bucketize(
                    pos_mae.clamp(max=float(bin_edges[-1])),
                    bin_edges[1:-1]
                ).long()
                local_uq_loss = torch.nn.functional.cross_entropy(local_uq_logits[out_mask], bin_target)
            else:
                pos_log_var = local_log_var[out_mask]
                local_uq_loss = (torch.exp(-pos_log_var) * pos_mae + pos_log_var).mean()
            loss = loss + self.uq_local_loss_weight * local_uq_loss

        if (global_quality is not None) and (self.uq_global_loss_weight > 0):
            pred_prob = torch.softmax(output[out_mask], dim=1).detach()
            true_prob = y[y_mask].detach()
            mae_score = 1.0 - torch.clamp(torch.abs(pred_prob - true_prob).mean(), 0.0, 1.0)
            vx = pred_prob.flatten() - pred_prob.flatten().mean()
            vy = true_prob.flatten() - true_prob.flatten().mean()
            pcc = torch.sum(vx * vy) / (torch.sqrt(torch.sum(vx * vx) * torch.sum(vy * vy)) + 1e-8)
            pcc_score = 0.5 * (pcc + 1.0)
            if self.uq_global_target == "mae_pcc":
                target = torch.stack((mae_score, pcc_score))
            elif self.uq_global_target == "pcc_flat":
                target = pcc_score.view(1)
            else:
                raise ValueError("Unknown uq_global_target: {}".format(self.uq_global_target))
            global_uq_loss = torch.nn.functional.mse_loss(torch.sigmoid(global_quality), target)
            loss = loss + self.uq_global_loss_weight * global_uq_loss
        #else:
    ...
    def endTraining(self, message="Training Successfully Ended."):
        ...
        MH.write(json.dumps(self.metrics_history, indent=2, default=jsonSafe))
        MH.close()
