...
def _processData(data_files, nc, labels_key, 
    ...
    for f in data_files:
        ...
        optional_template_tensors = {}
        if "template_x_dna" in data_arrays.files:
            optional_template_tensors["template_x_dna"] = torch.tensor(
                data_arrays["template_x_dna"], dtype=torch.float32
            )
        if "template_mask" in data_arrays.files:
            optional_template_tensors["template_mask"] = torch.tensor(
                data_arrays["template_mask"], dtype=torch.bool
            )
        if "template_scores" in data_arrays.files:
            optional_template_tensors["template_scores"] = torch.tensor(
                data_arrays["template_scores"], dtype=torch.float32
            )
        if "template_node_x" in data_arrays.files:
            optional_template_tensors["template_node_x"] = torch.tensor(
                data_arrays["template_node_x"], dtype=torch.float32
            )
        if "template_node_mask" in data_arrays.files:
            optional_template_tensors["template_node_mask"] = torch.tensor(
                data_arrays["template_node_mask"], dtype=torch.bool
            )
